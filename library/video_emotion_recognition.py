...
def gen():
    ...
    while end - start < max_time :
        ...
        for (i, rect) in enumerate(rects):
            ...
            Displease_0.append(prediction[0][0].astype(float))
            Disinterested_1.append(prediction[0][1].astype(float))
            Nervous_2.append(prediction[0][2].astype(float))
            happy_3.append(prediction[0][3].astype(float))
            Unsure_4.append(prediction[0][4].astype(float))
            Confused_5.append(prediction[0][5].astype(float))
            ...
            cv2.putText(frame, "Displease : " + str(round(prediction[0][0],3)),(40,140 + 180*i), cv2.FONT_HERSHEY_SIMPLEX, 0.5, 155, 0)
            cv2.putText(frame, "Disinterested : " + str(round(prediction[0][1],3)),(40,160 + 180*i), cv2.FONT_HERSHEY_SIMPLEX, 0.5, 155, 0)
            cv2.putText(frame, "Nervous : " + str(round(prediction[0][2],3)),(40,180 + 180*i), cv2.FONT_HERSHEY_SIMPLEX, 0.5, 155, 1)
            cv2.putText(frame, "Happy : " + str(round(prediction[0][3],3)),(40,200 + 180*i), cv2.FONT_HERSHEY_SIMPLEX, 0.5, 155, 1)
            cv2.putText(frame, "Unsure : " + str(round(prediction[0][4],3)),(40,220 + 180*i), cv2.FONT_HERSHEY_SIMPLEX, 0.5, 155, 1)
            cv2.putText(frame, "Confused : " + str(round(prediction[0][5],3)),(40,240 + 180*i), cv2.FONT_HERSHEY_SIMPLEX, 0.5, 155, 1)
            ...
            if prediction_result == 0 :
                cv2.putText(frame, "Displease",(x+w-10,y-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            elif prediction_result == 1 :
                cv2.putText(frame, "Disinterested",(x+w-10,y-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            elif prediction_result == 2 :
                cv2.putText(frame, "Nervous",(x+w-10,y-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            elif prediction_result == 3 :
                cv2.putText(frame, "Happy",(x+w-10,y-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            elif prediction_result == 4 :
                cv2.putText(frame, "Unsure",(x+w-10,y-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            elif prediction_result == 5 :
                cv2.putText(frame, "Confused",(x+w-10,y-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            else :
        ...
        # Emotion mapping
        #emotion = {0:'Displease', 1:'Disinterested', 2:'Nervous', 3:'Happy', 4:'Neutral', 5:'Unsure', 6:'Confused'}
        ...
        if end-start > max_time - 1 :
            ...
            rows = zip(Displease_0,Disinterested_1,Nervous_2,happy_3,Unsure_4,Confused_5,neutral_6)
...
# K.clear_session()
    ...
    # d.write(','.join(str(i) for i in Displease_0)+'\n')
    # d.write(','.join(str(i) for i in Disinterested_1)+'\n')
    #d.write(','.join(str(i) for i in Nervous_2)+'\n')
    # d.write(','.join(str(i) for i in happy_3)+'\n')
    #  d.write(','.join(str(i) for i in Unsure_4)+'\n')
    #  d.write(','.join(str(i) for i in Confused_5)+'\n')
